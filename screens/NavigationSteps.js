...
import { View, StyleSheet, TextInput, Touchable, TouchableOpacity, Text, SafeAreaView, ScrollView, Checkbox, StatusBar} from 'react-native';
...
const NavigationSteps = ({route}) => {
  ...
  useLayoutEffect(() => {
    ...
    const stepsToDisplay = () => {
        ...
        console.log("HIIIII" , startLocation)
    ...
    const nextStep = () => {
        ...
        if (steps[i].childHtmlSteps != null){
          ...
          console.log("hello")
        }else {setChildSteps(null)}
        if (steps[i].parentHtmlStep != null){
          setParentStep(steps[i].parentHtmlStep)
        }else{setParentStep(null)}
      }else {navigation.navigate("Arrived Screen")}
    }
    ...
    return(
      <SafeAreaView style = {styles.background} >
        <View style={styles.header}>
          ...
          <TouchableOpacity style = {styles.headerFour} onPress = {() => navigation.navigate('Map Display Screen', {start: startLocation, end: endLocation, mode: transport})}>
      ...
      {parentStep ? <Text style = {styles.parent}>{parentStep.replace(/<\/?[^>]+(>|$)/g, " ")}</Text> : <Text></Text>} 
      {childSteps ? <Text style = {styles.instructions}>Directions:</Text> : <Text></Text>}
      
    </View>
    
    <ScrollView>
    
      {childSteps ? <TouchableOpacity style={styles.contact} >
            <Text style = {styles.steps}> {childSteps[0].replace(/<\/?[^>]+(>|$)/g, " ")} </Text>
          </TouchableOpacity>  : <Text></Text>}
         
          {childSteps != null && childSteps[1] != null ?  <TouchableOpacity style = {styles.headerTwo} onPress = {() => {setChildSteps(childSteps.slice(1))}}>
          <Text style = {styles.headerTextStyleTwo}>Next Direction</Text >
          </TouchableOpacity>  : <TouchableOpacity style = {styles.headerThree} onPress = {() => nextStep()}>
