...
const Navigation = ({ route }) => {
  ...
  const fetchUrl = async () => {
    ...
    const url = `https://maps.googleapis.com/maps/api/directions/json?destination=${DestinationAddress.replace(
      /\s/g,
      "%20"
    )}&mode=${transport}&origin=${originAddress.replace(
      /\s/g,
      "%20"
    )}&key=AIzaSyD85mtgaRF3QYxdniTj-6NrIjOeLQIR_rE`;
  ...
  const getStructuredStepsFromData = (data) => {
    if (
      ...
      console.log(
        "HHEEERREEE",
        structuredSteps[structuredSteps.length - 1].end
      );
      navigation.navigate("Navigation Steps Screen", {
        steps: structuredSteps,
        mode: transport,
      });
    } else {
      Alert.alert(
        "Unable to find Directions. Make sure you include street name or city for more accurate results",
        "",
  ...
  return (
    <SafeAreaView style={styles.background}>
      ...
      {location == null && transport != null && useLocation == true ? (
      ...
      {useLocation == null && transport != null ? (
        <View style={styles.locationView}>
          ...
          <Text style={styles.nav}>
            Use your current location for the starting point for navigation
          </Text>
        </View>
      ) : (
        <Text></Text>
      ...
      {(useLocation == false && transport != null && confirm != true) ||
      (useLocation != null &&
        transport != null &&
        confirm != true &&
        useLocation == false) ? (
      ...
      (useLocation != null &&
        ...
        confirm != true &&
        useLocation == false) ? (
        <TouchableOpacity style={styles.headerTwo} onPress={() => fetchUrl()}>
      ...
      (useLocation == false && confirm == true) ? (
        <View style={styles.confirm}>
          {useLocation == true && location != null ? (
            <View>
              <Text style={styles.detailsTwo}>Start Location: </Text>
              <Text style={styles.details}>Your Location</Text>
            </View>
          ) : (
            <View>
              <Text style={styles.detailsTwo}>Start Location: </Text>
              <Text style={styles.details}>{originAddress}</Text>
            </View>
          )}
          ...
          <TouchableOpacity
            style={styles.headerFive}
            onPress={() => getStructuredStepsFromData(directionsData)}
          >
