...
import { View, Text, TextInput, Button, StyleSheet, SafeAreaView, Alert, Platform, StatusBar, Image, TouchableOpacity, TouchableWithoutFeedbackBase} from 'react-native'
...
const GoalScreen = () => {
  ...
  const newGoals = async () => {
    ...
    if (goalRatings != null){
          ...
          try {
            ...
            await AsyncStorage.setItem('@ThisWeeksGoals', JSON.stringify(currentGoals))
          } catch (e) {
            console.log(e)
            console.log("this")
            // saving error
  ...
  const dayCount = async () => {
    ...
    try {
      ...
      if (7 - time == 0){
        navigation.navigate("Week Finished Screen", {goalRatings: goalRatings, thisWeeksGoals: thisWeeksGoals, goalCompletion: goalCompletion })
      }
  ...
  const saveGoal = async (goal, value) => {
    ...
    setGoalCompletion({goalOne: goalOne, goalTwo: goalTwo, goalThree: goalThree})
    const set_goals = {goalOne: goal_one, goalTwo: goal_two, goalThree: goal_three}
    ...
    if (complete == true){
      if (goal_one == true && goal_two == true && goal_three == true) {
        setGone(true)
        navigation.navigate("Trophy Complete", {set_goals: set_goals, goal: goal, goalComplete: goalComplete, goalName: goalName, goalRatings: goalRatings})
      }
      else{
        setGone(true)
        navigation.navigate("Goal Complete Screen", {set_goals: set_goals, goal: goal, goalComplete: goalComplete, goalName: goalName, goalRatings: goalRatings})
      }
      }
  ...
  useEffect(() => {
    ...
    if (7 - time == 0){
      navigation.navigate("Week Finished Screen", {goalRatings: goalRatings, thisWeeksGoals: thisWeeksGoals })
  ...
  return (
    <SafeAreaView style = {styles.background}>
      <View style={styles.header}>
          ...
          <TouchableOpacity style = {styles.progress} onPress = {() => navigation.navigate("Week Finished Screen", {goalRatings: goalRatings, thisWeeksGoals: thisWeeksGoals, goalCompletion: goalCompletion })
        }>
          <Text style = {styles.headerTextStyleTwo}>Progress</Text >
          </TouchableOpacity> : <TouchableOpacity style = {styles.headerTwo} onPress = {() => {navigation.navigate('Achievement Screen'); setGone(true)}}>
      ...
      <TouchableOpacity style = {styles.goalList} onPress = {() => {navigation.navigate("View Goal Screen", { name : goalRatings[thisWeeksGoals[0]].name,
      ...
      goalRatings: goalRatings}), setGone(true)}}>
      
      {thisWeeksGoals != 'null' && thisWeeksGoals != null && goalRatings != null && thisWeeksGoals != ' ' ? <Text style = {styles.text}>{goalRatings[thisWeeksGoals[0]].name}</Text> : <Text>LOADING...</Text>}
      ...
      <View style = {styles.goal}>
      <TouchableOpacity style = {styles.goalList} onPress = {() => {navigation.navigate("View Goal Screen", { name : goalRatings[thisWeeksGoals[1]].name,
      ...
      goalRatings: goalRatings}); setGone(true)}}>
      {thisWeeksGoals != 'null' && thisWeeksGoals != null && thisWeeksGoals.length > 1 && goalRatings != null && thisWeeksGoals != ' ' ? <Text style = {styles.text}>{goalRatings[thisWeeksGoals[1]].name}</Text> : <Text>LOADING...</Text>}
      ...
      <View style = {styles.goal}>
      <TouchableOpacity style = {styles.goalList} onPress = {() => {navigation.navigate("View Goal Screen", { name : goalRatings[thisWeeksGoals[2]].name,
      ...
      goalRatings: goalRatings}); setGone(true)}}>
      {thisWeeksGoals != 'null' && thisWeeksGoals != null && thisWeeksGoals.length > 2 && goalRatings != null && thisWeeksGoals != ' ' ? <Text style = {styles.text}>{goalRatings[thisWeeksGoals[2]].name}</Text> : <Text>LOADING...</Text>}
      ...
      <Text style = {styles.textTwo}>Check the box to complete the goal.{'\n'} Touch the goal to view/add details.</Text>
